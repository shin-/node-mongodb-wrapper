...
var Database = exports.Database = function(host, port, name, prefix, auth) {
    ...
    function connectionOpened(err, connection) {
        ...
        if (!err && auth && auth.user && auth.password) {
            return connection.authenticate(auth.user, auth.password, function(err) {
                if (err) exports.log(" !!! Mongo (" + err + ")")
...
exports.db = function(host, port, name, prefix, auth) {
    return new Database(host, port, name, prefix, auth)
}   
